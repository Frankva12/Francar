...
$libros = new Libros();
$suma = 0;
$pdf->head('REPORTE DE VENTAS POR EDITORIAL');
$pdf->date();
$pdf->SetFont('Arial', 'B', '10');
$pdf->SetFillColor(36, 113, 163);
$pdf->SetTextColor(255,255,255);
$pdf->Cell(50);
$pdf->Cell(47,10, utf8_decode('Editorial'),0,0,'C', true);
$pdf->Cell(47,10, utf8_decode('Precio'),0,0,'C', true);

$pdf->Ln();

if($libros ->setId($_SESSION['id_administrador'])){
    $data = $libros->cantidad_ventas_editorial();
    foreach($data as $datos){
        $pdf->setFont('Arial','','10');
        $pdf->SetFillColor(255,255,255);
        $pdf->SetTextColor(0,0,0);
        $pdf->Cell(50);
        $pdf->Cell(47,10, utf8_decode($datos['nombre_editorial']), 'B, T',0,'C',true);
